...
class BrowseController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $url = "http://localhost:8983/solr/mal_core/select?indent=true&q.op=OR&q=title_txt_en%3A" . ubahSearchKey($request->search) . "%0Abody_txt_en%3A" . ubahSearchKey($request->search) . "%0Agenre_txt_en%3A" . ubahSearchKey($request->search);
        // supaya data yang tampil bisa lebih dari default
        $url = $url . "&rows=50";
        ...
        if ($fixed_result['response']['numFound'] > 5) {
            $setResult = $fixed_result['response']['numFound'];
            if ($setResult > 50) {
                $setResult = 50;
            }
            // hitung banyak halaman yang akan ditampilkan dari data yang ditemukan
            $banyakHalaman = penghitungPaginate($setResult, $perPage);
        ...
        $previousSearch = ubahSearchKey($request->search);
